	...
	for(let i = 0; i < tiles.length; i++){
		...
		// 2 == 2
 		if(tiles[i] == tiles[i+1]){
            ...
            // score = score + tiles[i]; - shorthands
            score += tiles[i];
		}
	}
...
function slideLeft(){
	for(let r=0; r<rows; r++){
        ...
        // this is line for the animation - copy the content of the original row
		...
		for(let c=0; c<columns; c++){
            ...
            if (originalRow[c] !== num && num !== 0) {
                ...
                // remove the animation after animation is complete
                setTimeout(() => {
...
// event listener to capture touch in the screen and assigns the x and y coordinates in startX and startY
document.addEventListener("touchstart", (e) =>{
    startX = e.touches[0].clientX;
    startY = e.touches[0].clientY;
})

// event listener to check where you touch the screen and prevents scrolling - input targets any element that includes the word title
...
document.addEventListener("touchend", (e) => {
    ...
    // calculate the difference betrween the initial touch position and the final touch
